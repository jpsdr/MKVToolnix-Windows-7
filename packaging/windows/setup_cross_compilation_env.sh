...
function update_mingw_cross_env {
  if [[ ! -d $INSTALL_DIR ]]; then
    ...
    git clone --branch Boost_Fix https://github.com/jpsdr/mxe-Windows-7.git $INSTALL_DIR >> $LOGFILE 2>&1
  else
    echo Updating the M cross environment build scripts >> $LOGFILE
    cd $INSTALL_DIR
    git fetch >> $LOGFILE 2>&1 \
      && git reset --hard origin/Boost_Fix >> $LOGFILE 2>&1 \
      && git config branch.$(git branch --show-current).merge refs/heads/Boost_Fix >> $LOGFILE 2>&1
  fi
