...
void QgsCollapsibleGroupBoxBasic::showEvent( QShowEvent *event )
{
  ...
  else
  {
    ...
    Q_EMIT collapsedStateChanged( isCollapsed() );
  }
...
//    offsetTopTri += rectTitle.top();
  ...
  if ( QApplication::style()->objectName().compare( QLatin1String( "oxygen" ), Qt::CaseInsensitive ) == 0 )
  {
    ...
    initStyleOption( &box );
    const QRect rectFrame2 = style()->subControlRect( QStyle::CC_GroupBox, &box, QStyle::SC_GroupBoxFrame, this );
    const QRect rectCheckBox = style()->subControlRect( QStyle::CC_GroupBox, &box,
                               QStyle::SC_GroupBoxCheckBox, this );
    if ( rectFrame2.left() <= 0 )
      offsetLeft = 6 + rectFrame2.left();
    if ( rectFrame2.top() <= 0 )
    {
      if ( isCheckable() )
      {
  ...
  {
    ss += QLatin1String( "mtx--gui--Util--QgsCollapsibleGroupBoxBasic, mtx--gui--Util--QgsCollapsibleGroupBox {" );
    ss += QStringLiteral( "  margin-top: %1px;" ).arg( topBuffer + rectFrame.top() );
    ss += '}';
  }
  ss += QLatin1String( "mtx--gui--Util--QgsCollapsibleGroupBoxBasic::title, mtx--gui--Util--QgsCollapsibleGroupBox::title {" );
  ...
  QString ssd;
  ssd = QStringLiteral( "mtx--gui--Util--QgsCollapsibleGroupBoxBasic > QToolButton#%1, mtx--gui--Util--QgsCollapsibleGroupBox > QToolButton#%1 {" ).arg( mCollapseButton->objectName() );
  ssd += QLatin1String( "  background-color: rgba(255, 255, 255, 0); border: none;" );
  ssd += QStringLiteral( "} mtx--gui--Util--QgsCollapsibleGroupBoxBasic > QToolButton#%1:focus, mtx--gui--Util--QgsCollapsibleGroupBox > QToolButton#%1:focus {  border: 1px solid palette(highlight); }" ).arg( mCollapseButton->objectName() );
...
{
  if ( !mSaveCollapsedState && !mSaveCheckedState )
    return;
  ...
  auto settings = Util::Settings::registry();

  if ( mSaveCheckedState )
  {
    const QVariant val = settings->value( key + "/checked" );
    if ( ! val.isNull() )
      setChecked( val.toBool() );
  }
  if ( mSaveCollapsedState )
  {
    const QVariant val = settings->value( key + "/collapsed" );
    if ( ! val.isNull() )
...
{
  if ( !mShown || ( !mSaveCollapsedState && !mSaveCheckedState ) )
    return;
  ...
  if ( mSaveCheckedState )
    settings->setValue( key + QStringLiteral( "/checked" ), isChecked() );
  if ( mSaveCollapsedState )
    settings->setValue( key + QStringLiteral( "/collapsed" ), isCollapsed() );
}
